...
class HibriModel():
    ...
    def predict(self, x, selected_model='Ensemble'):
        ...
        # Convert the one-hot encoded features to a tensor
        ...
        for i in range( ensemble_predictions.shape[0] ):            
            ...
            if count == ensemble_predictions.shape[1]:
                if predicted_mode == round( y_hat[i] ): confidence = 1.0
                else: confidence = 0.0
            else:
                if predicted_mode != round( y_hat[i] ): confidence = 0.0
                else: confidence = count / ensemble_predictions.shape[1]
            final_confidence_score.append( confidence )
        final_confidence_score = np.array( final_confidence_score )

        # Create a DataFrame with the predictions and confidence levels
